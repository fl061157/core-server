...
//    @Column(name = "avatar_url")
    /**
     * 解决cansandra om框架托管，
     * 直接读取字段取值，不读取重写getAvatar_url的问题
     * avatar_url 没有入库，只有avatar字段是入库
     */
            ...
            if (null != avatar && !"".equals(avatar.trim())) {
                if (avatar.startsWith("http:") || avatar.startsWith("https:")) {
                    ...
                    if ("0086".equals(countrycode) || "+86".equals(countrycode) || "86".equals(countrycode)) {
                        return cnCDNUrl + "/api/avatar/" + avatar;
                    } else {
                        return usCDNUrl + "/api/avatar/" + avatar;
                    }
                }
            }
        } else {
    ...
    private static String cnCDNUrl;

    private static String usCDNUrl;

    public static void init(String cnCDNUrlPar, String usCDNUrlPar) {
        cnCDNUrl = cnCDNUrlPar;
        usCDNUrl = usCDNUrlPar;
    }
}
